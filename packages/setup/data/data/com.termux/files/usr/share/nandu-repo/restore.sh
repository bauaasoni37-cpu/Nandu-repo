...
# Restores the exact Nandu dev environment: packages, SDK config, env, wrappers.
...
echo -e "${CYAN}==> [2/6] Installing exact Ubuntu dev packages ...${RESET}"
if [ -s "$SHARE/ubuntu-packages.list" ]; then
    PKG_LIST=$(tr '\n' ' ' < "$SHARE/ubuntu-packages.list")
    # shellcheck disable=SC2086
    apt-get install -y --no-install-recommends $PKG_LIST 2>&1 | tail -5 || true
